...
pathbin="/totvs/protheus/bin/apprest"
    ...
    if [ -z "$pid" ]; then
        ...
        if [ ${RETVAL} -eq 0 ]; then
            ...
            if [ -n "$pid" ]; then
                ...
                echo "PID : $pid"
                echo -e "${prog} Rest running :   ${green}[ OK ]${reset}"
            else
                echo -e "Failed to get PID for Rest ${prog} : ${red}[ Failure ]${reset}"
            fi
        else
            echo -e "Failed to start Rest ${prog} :         ${red}[ Failure ]${reset}"
        fi
        echo
    else
        echo -e "$prog Rest is ${green}Started${reset} pid $pid"
    fi
}
...
stop_service() {
    pid=$(get_pid)
    if [ -z "$pid" ]; then
        echo -e "${prog} Rest is not running ${red}[ Stopped ]${reset}"
        return 0
    fi
    ...
    echo
    echo -e "$prog Rest is Stopped     ${red}[ Stopped ]${reset}"
}
...
kill_service() {
    ...
    echo
    echo -e "$prog Rest is Killed     ${red}[ Killed ]${reset}"
}
...
get_stats() {
    ...
    if [ -n "$pid" ]; then
        ...
        echo -e "${red}- Program $prog Rest is not running! ${reset}"
    fi
}
